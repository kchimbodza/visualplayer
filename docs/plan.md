...
- [ ] Audio chip and track notice (`tools_row.lua`, `track_notice.lua`): an outlined chip just left of the output chip shows the current track's format and number, like "Dolby Digital 5.1 · 2/5", opening the audio and subtitles menu when clicked, and hidden for files with one audio track. Whenever the track changes, a one-line notice appears above the controls on the left for 3 seconds, like "Audio 2 of 5 · English   Dolby Digital 5.1 · AC-3 · Passthrough", and the controls come up with it.
