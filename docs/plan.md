...
- **8K is labeled 8K.** The resolution labels stopped at 4K, so 7680×4320 video showed "4K" until 8K was added (6400 wide or 4000 tall and above). Found with Jellyfin's 8K test files.
- **Drag the picture to move the window.** A press where no control is (`click_area.lua`) switches mpv's `window-dragging` on for that press only and calls `begin-vo-dragging`, so the window moves from anywhere on the picture, not just the top bar, while the seek bar keeps working. Double-clicking the picture still toggles fullscreen. With touch controls on (as on the PX13, where a touchscreen is detected), `video_taps.lua` tells a tap from a drag by whether the pointer moves more than 10 design pixels while pressed, since mpv can't tell a mouse from a finger: a tap shows or hides the controls, and a drag moves the window. The first version treated every press on the picture as a tap there, so dragging didn't work even with a mouse.
- **Every run saves a log.** `visualplayer` passes mpv's `--log-file`, writing `~/.local/state/visual-player/last.log` (replaced each run), so there's a log for testing however a video was opened. Before release, consider making this a setting.
- **The launcher starts Visual Player with no file.** Plain mpv with no file prints its usage and quits, so opening from GNOME's app grid did nothing. `visualplayer` now passes `--idle=once --force-window=yes`, and `idle_screen.lua` shows Visual Player's name, "Your videos, in the picture and sound they were made with", and the formats it handles as outlined badges, picture on one row (4K, HDR10, Dolby Vision, HLG, AV1, HEVC) and sound on the next (Dolby Atmos, Dolby TrueHD, Dolby Digital Plus, Dolby Digital, DTS-HD, DTS, 7.1 Surround); not HDR10+, which mpv doesn't report, or Dolby Surround, which is a soundbar's upmixing mode rather than a format until something plays, with a quieter "Drop a video here to start watching" at the bottom of the window (version 0.6.1).
...
- [ ] Audio chip and track notice (`tools_row.lua`, `track_notice.lua`): an outlined chip just left of the output chip names the current track the way a soundbar or streaming app would, with its number, like "Dolby Digital 5.1 · 2/5" or "Dolby Atmos 7.1 · 1/5"; everyday formats name the codec with its channels, like "Opus Stereo" or "AAC 5.1". The format badges use the same names, as one badge for the format and channels ("Dolby Digital 5.1"), after a separate "Dolby Atmos" badge for Atmos tracks. It's informational only, with no hover or click. It always shows when there's sound, with the track number only when there's more than one track, like "AAC Stereo". Whenever the track changes, a one-line notice appears above the controls on the left for 3 seconds, like "Audio 2 of 5 · English   Dolby Digital 5.1 · AC-3 · Passthrough", and the controls come up with it.
